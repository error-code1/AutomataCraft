...
textArea.setAttribute('placeholder', 'Result...');
textArea.readOnly=true;
textArea.style.width = '100%'; // Set the width to 100% for full width
...
fa.import(load());
render();
function updateOutputArea(output,col=1) {
    const outputArea = document.getElementById('my-text-area');
    outputArea.value = output;
    if(col==2) outputArea.style.color='green';
    if(col==3) outputArea.style.color='red';
    if(col==1) outputArea.style.color='black';
}
$('#test').onclick = function () {
    try {
        ...
        const string = window.prompt('Enter input string : ');
        
        if (string && string.trim()) {
            const res=regex.test(string.trim()) ? 'Accepted' : 'Failed';
            v=+(res[0]=='A');
            console.log(v);
            updateOutputArea(res,3-v);
        }
    } catch (e) {
        handleError(e);
    }
...
$('#reset').onclick = function () {
    if (!confirm('Are you sure? Your progress will be removed')) return;
...
$('#minimizedfa').onclick = () => {
    try {
        ...
        render();
        updateOutputArea('Succesfully Minimized',2);
    } catch (e) {
        handleError(e);
    }
};
...
$('#convert2re').onclick = () => {
    try {
        ...
        updateOutputArea(`Result: ${symbol}`);
        // confirm(`Result : ${symbol} `);
    } catch (e) {
        handleError(e);
    }
};
            ...
            items: [
                {
                    ...
                    onclick: () => {
                        ...
                        const name = prompt('Enter the name of the state ');
